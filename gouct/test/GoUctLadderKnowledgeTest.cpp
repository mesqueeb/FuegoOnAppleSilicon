...
void GoUctLadderKnowledgeTestInit(std::vector<SgUctMoveInfo>& moves, const GoBoard& bd)
{
    for (GoBoard::Iterator it(bd); it; ++it)
        moves.push_back(SgUctMoveInfo(*it));
}

void GoUctLadderKnowledgeTestCheckValue(const SgUctMoveInfo& moveInfo, SgPoint p,
...
void GoUctLadderKnowledgeTestCheckUndefined(const SgUctMoveInfo& moveInfo, SgPoint p)
{
    BOOST_CHECK_EQUAL(moveInfo.m_move, p);
...
const SgUctMoveInfo* GoUctLadderKnowledgeTestFindMoveInfo(const std::vector<SgUctMoveInfo>& moves, 
                            SgPoint p)
{
    /* @todo write it using a find template.
    std::vector<SgUctMoveInfo>::iterator it = find(moves, p);
    BOOST_REQUIRE_NE(it, moves.end());
	GoUctLadderKnowledgeTestCheckValue(*it, p, value, count);
    */
...
void GoUctLadderKnowledgeTestCheckValue(const std::vector<SgUctMoveInfo>& moves, SgPoint p,
                SgUctValue value, SgUctValue count)
{
    const SgUctMoveInfo* m = GoUctLadderKnowledgeTestFindMoveInfo(moves, p);
    if (m)
    	GoUctLadderKnowledgeTestCheckValue(*m, p, value, count);
}

void GoUctLadderKnowledgeTestCheckUndefined(const std::vector<SgUctMoveInfo>& moves, SgPoint p)
{
    const SgUctMoveInfo* m = GoUctLadderKnowledgeTestFindMoveInfo(moves, p);
    if (m)
		GoUctLadderKnowledgeTestCheckUndefined(*m, p);
}
//----------------------------------------------------------------------------
...
BOOST_AUTO_TEST_CASE(GoUctLadderKnowledgeTest_GoodLadderEscapeBonus)
{
    ...
    GoUctLadderKnowledgeTestInit(moves, bd);
    tester.ProcessPosition(moves);
    
    // ladder escape move
    GoUctLadderKnowledgeTestCheckValue(moves, Pt(3,5), SgUctValue(1), 
               SgUctValue(GOOD_LADDER_ESCAPE_BONUS));
    GoUctLadderKnowledgeTestCheckUndefined(moves, Pt(4,5));
}
...
BOOST_AUTO_TEST_CASE(GoUctLadderKnowledgeTest_BadLadderEscapePenalty)
{
    ...
    GoUctLadderKnowledgeTestCheckValue(moves, Pt(3,5), SgUctValue(0), 
...
BOOST_AUTO_TEST_CASE(GoUctLadderKnowledgeTest_LadderCaptureBonus)
{
    ...
    // ladder escape move
    GoUctLadderKnowledgeTestCheckValue(moves, Pt(3,4), SgUctValue(1), 
...
BOOST_AUTO_TEST_CASE(GoUctLadderKnowledgeTest_Good2LibTacticsLadderBonus)
{
    ...
    GoUctLadderKnowledgeTestCheckValue(moves, Pt(3,2), SgUctValue(1), 
               SgUctValue(GOOD_2_LIB_TACTICS_LADDER_BONUS));
    // not for plain ladder escape by extending.
    GoUctLadderKnowledgeTestCheckUndefined(moves, Pt(6,2));
}
